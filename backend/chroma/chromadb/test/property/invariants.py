...
from chromadb.api import custom_types
...
def wrap_all(record_set: RecordSet) -> NormalizedRecordSet:
    ...
    embedding_list: Optional[custom_types.Embeddings]
    if record_set["embeddings"] is None:
        ...
        if len(record_set["embeddings"]) > 0:
            if all(
                ...
                embedding_list = cast(custom_types.Embeddings, record_set["embeddings"])
            elif all(
...
def _exact_distances(
    query: custom_types.Embeddings,
    targets: custom_types.Embeddings,
...
def ann_accuracy(
    ...
    embedding_function: Optional[custom_types.EmbeddingFunction] = None,  # type: ignore[type-arg]
    query_indices: Optional[List[int]] = None,
    query_embeddings: Optional[custom_types.Embeddings] = None,
) -> None:
    ...
    embeddings: Optional[custom_types.Embeddings] = normalized_record_set["embeddings"]
...
def _query_results_are_correct_shape(
    query_results: custom_types.QueryResult, n_results: int
) -> None:
