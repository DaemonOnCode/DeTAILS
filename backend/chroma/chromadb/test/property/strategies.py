...
import chroma.chromadb.api.custom_types as custom_types
import re
...
from chromadb.custom_types import LiteralValue, WhereOperator, LogicalOperator
...
class RecordSet(TypedDict):
    ...
    ids: Union[custom_types.ID, List[custom_types.ID]]
    embeddings: Optional[Union[custom_types.Embeddings, custom_types.Embedding]]
    metadatas: Optional[Union[List[Optional[custom_types.Metadata]], custom_types.Metadata]]
    documents: Optional[Union[List[custom_types.Document], custom_types.Document]]
...
class NormalizedRecordSet(TypedDict):
    ...
    ids: List[custom_types.ID]
    embeddings: Optional[custom_types.Embeddings]
    metadatas: Optional[List[Optional[custom_types.Metadata]]]
    documents: Optional[List[custom_types.Document]]
...
class StateMachineRecordSet(TypedDict):
    ...
    ids: List[custom_types.ID]
    embeddings: custom_types.Embeddings
    metadatas: List[Optional[custom_types.Metadata]]
    documents: List[Optional[custom_types.Document]]
...
class Record(TypedDict):
    ...
    id: custom_types.ID
    embedding: Optional[custom_types.Embedding]
    metadata: Optional[custom_types.Metadata]
    document: Optional[custom_types.Document]
...
class hashing_embedding_function(custom_types.EmbeddingFunction[Documents]):
    def __init__(self, dim: int, dtype: npt.DTypeLike) -> None:
        self.dim = dim
        self.dtype = dtype

    def __call__(self, input: custom_types.Documents) -> custom_types.Embeddings:
...
class not_implemented_embedding_function(custom_types.EmbeddingFunction[Documents]):
...
) -> st.SearchStrategy[custom_types.EmbeddingFunction[Embeddable]]:
    return st.just(
...
class ExternalCollection:
    ...
    name: str
    metadata: Optional[custom_types.Metadata]
    embedding_function: Optional[custom_types.EmbeddingFunction[Embeddable]]
...
class Collection(ExternalCollection):
    ...
    dimension: int
    dtype: npt.DTypeLike
    known_metadata_keys: custom_types.Metadata
...
@st.composite
def document(draw: st.DrawFn, collection: Collection) -> custom_types.Document:
...
@st.composite
def where_clause(draw: st.DrawFn, collection: Collection) -> custom_types.Where:
...
@st.composite
def where_doc_clause(draw: st.DrawFn, collection: Collection) -> custom_types.WhereDocument:
...
def binary_operator_clause(
    base_st: SearchStrategy[custom_types.Where],
) -> SearchStrategy[custom_types.Where]:
...
def binary_document_operator_clause(
    base_st: SearchStrategy[custom_types.WhereDocument],
) -> SearchStrategy[custom_types.WhereDocument]:
    op: SearchStrategy[LogicalOperator] = st.sampled_from(["$and", "$or"])
    return st.dictionaries(
        keys=op,
        values=st.lists(base_st, max_size=2, min_size=2),
...
@st.composite
def recursive_where_clause(draw: st.DrawFn, collection: Collection) -> custom_types.Where:
    base_st = where_clause(collection)
    where: custom_types.Where = draw(st.recursive(base_st, binary_operator_clause))
    return where
...
) -> custom_types.WhereDocument:
    base_st = where_doc_clause(collection)
    where: custom_types.WhereDocument = draw(
...
class Filter(TypedDict):
    where: Optional[custom_types.Where]
    ids: Optional[Union[str, List[str]]]
    where_document: Optional[custom_types.WhereDocument]
...
def filters(
    ...
    ids: Optional[Union[List[custom_types.ID], custom_types.ID]]
