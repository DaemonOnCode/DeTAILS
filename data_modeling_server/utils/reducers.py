...
from models.table_dataclasses import CodebookType, GenerationType, InitialCodebookEntry, KeywordEntry, QectResponse, ResponseCreatorType
...
def process_sampled_post_response_action(workspace_id: str, action: Dict[str, Any]) -> None:
    ...
    if action_type == "SET_CORRECT":
        ...
        # If invalid, no-op (matches frontend returning unchanged state with empty concat)

    elif action_type == "SET_RESPONSES":
        # Clear existing state (matches frontend replacement)
        ...
        if responses:
            ...
            # Update chatHistory if provided
            if chat_history is not None:
                updates["chat_history"] = json.dumps(chat_history)

            # Update code if provided and non-empty
        ...
        # No-op as in frontend (or implement specific rerun logic if needed)
        pass

    else:
        # No-op for unknown actions, matching frontend default case
        pass
...
def process_initial_codebook_table_action(workspace_id: str, action: Dict[str, Any]) -> None:
    """
    Process actions for initialCodebookTable, updating the initial_codebook_repo accordingly.
    ...
    # initial_codebook_repo = SomeRepository()
