...
class CommentsRepository(BaseRepository[Comment]):
    ...
    def fetch_unprocessed_comments(self, dataset_id: str, batch_size: int, num_threads: int) -> List[Comment]:
        """
        ...
        query, params = query_builder.select("id", "body") \
            ...
            .where("id", 
                   self.execute_raw_query("SELECT comment_id FROM tokenized_comments", keys=True), 
                   operator="NOT IN") \
