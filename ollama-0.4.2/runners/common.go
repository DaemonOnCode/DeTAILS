...
func escapePath(path string) string {
	switch runtime.GOOS {
	case "windows":
		// On Windows, paths with spaces are enclosed in quotes
...
func Refresh(payloadFS fs.FS) (string, error) {
	...
	if hasPayloads(payloadFS) {
		...
		if err != nil {
			fmt.Println("Error in locateRunners", err)
		}
	}
...
func locateRunners() (string, error) {
	...
	fmt.Println("In locateRunners exe", exe)
	if err != nil {
		fmt.Println("Error in locateRunners exe", err)
		return "", err
	}

	cwd, err := os.Getwd()
	fmt.Println("In locateRunners cwd", cwd)
	if err != nil {
		fmt.Println("Error in locateRunners cwd", err)
		return "", err
	}
	...
	for _, path := range paths {
		fmt.Println("In locateRunners path", path)
		...
		if err == nil {
			...
			fmt.Println("Error in locateRunners stat candidate", err)
		}
	}
...
func hasPayloads(payloadFS fs.FS) bool {
	...
	fmt.Println("In hasPayloads", files, err)
...
func extractRunners(payloadFS fs.FS) (string, error) {
	fmt.Println("In extractRunners, calling cleanupTmpDirs")
...
func GetAvailableServers(payloadsDir string) map[string]string {
	fmt.Println("In GetAvailableServers", payloadsDir)
	if payloadsDir == "" {
