...
const InitialCodingPage = () => {
  ...
  const [references, setReferences] = useState<{ [code: string]: { text: string; postId: number; isComment: boolean }[] }>({});
  ...
  const applyCodeToSelection = () => {
    ...
    const isComment = (selectedPost?.comments ?? []).some(comment => comment.body.includes(selectedText || "")) || false;
  ...
  return (
    <div className="p-6 h-full flex justify-between flex-col">
        ...
        <NavigationBottomBar previousPage={ROUTES.WORD_CLOUD} nextPage={ROUTES.GENERATION} isReady={true}/>
    </div>
  );
};
