...
const WordCloud: React.FC<WordCloudProps> = ({ mainWord, words, selectedWords, toggleWordSelection }) => {
  ...
  useEffect(() => {
    ...
    const maxDistance = placedWords.reduce((max, word) => {
      const distance = Math.sqrt(word.x ** 2 + word.y ** 2) + Math.max(word.width, word.height) / 2;
  ...
  return (
    <div className="relative bg-gray-100 rounded-full shadow-lg" style={{ width: maxRadius * 2, height: maxRadius * 2 }}>
      <svg width={maxRadius * 2} height={maxRadius * 2} style={{ position: "absolute", top: 0, left: 0, zIndex: 0 }}>
        {wordsPlaced.map((word) => {
          ...
          return <line key={`line-${word.text}`} x1={mainWordX} y1={mainWordY} x2={wordX} y2={wordY} stroke="gray" strokeWidth="1" />;
        })}
      </svg>
